// DO NOT EDIT. This file is generated by Fresh.
// This file SHOULD be checked into source version control.
// This file is automatically updated during development when running `dev.ts`.

import * as $0 from "./routes/(auth)/_layout.tsx";
import * as $1 from "./routes/(dashboard)/[storeid].tsx";
import * as $2 from "./routes/(dashboard)/_layout.tsx";
import * as $3 from "./routes/_404.tsx";
import * as $4 from "./routes/_app.tsx";
import * as $5 from "./routes/api/joke.ts";
import * as $6 from "./routes/index.tsx";
import * as $$0 from "./islands/Counter.tsx";

const manifest = {
  routes: {
    "./routes/(auth)/_layout.tsx": $0,
    "./routes/(dashboard)/[storeid].tsx": $1,
    "./routes/(dashboard)/_layout.tsx": $2,
    "./routes/_404.tsx": $3,
    "./routes/_app.tsx": $4,
    "./routes/api/joke.ts": $5,
    "./routes/index.tsx": $6,
  },
  islands: {
    "./islands/Counter.tsx": $$0,
  },
  baseUrl: import.meta.url,
};

export default manifest;
